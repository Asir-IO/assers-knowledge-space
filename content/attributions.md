---
title: Attributions
thumbnail: "[[attributions-icon.svg]]"
---
I can confidently say that this Knowledge Space wouldn't have been possible if not for the open-source projects/assets that I had access to; I'm grateful for every human being who made each of them possible :pi-smile:.

Below is the complete list.

---
# Assets
1. {A} [Pixelarticons](https://pixelarticons.com/)
	- (**License**): [MIT](https://opensource.org/license/mit)
2.  [Google Material Icons](https://pixelarticons.com/)
	- (**License**): [Apache](https://www.apache.org/licenses/LICENSE-2.0.txt)
3.  [Iconify Icons Set](https://icon-sets.iconify.design/)
	- (**License**): [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
# Code Snippets
1. {A} [Terrain XOR, Limited Gradient](https://openprocessing.org/@u594485/2939122) by [qp91mn64](https://openprocessing.org/@u594485/#sketches)
	- (**License**): [CC BY-NC-SA 3.0](https://creativecommons.org/licenses/by-nc-sa/3.0/)
	- (**Usage**): the homepage's [[index#^banner|welcome banner animation]].
2. [An Interactive Guide to the Fourier Transform](https://betterexplained.com/examples/fourier/) by [Khalid Azad](https://betterexplained.com/about/)
	- (**License**): [CC BY-NC-SA 3.0](https://creativecommons.org/licenses/by-nc-sa/3.0/)
	- (**Usage**): a fourier series [[Harmonically Related Complex Exponentials#^vis|visualizer]].
# Tools
1. {A} [Obsidian](https://obsidian.md/)
	- I use this software ti store and create every single note.
	- Needless to say, it's what made all this possible.
2. [Obsidian's Excalidraw](https://community.obsidian.md/plugins/obsidian-excalidraw-plugin) by [Zsolt](https://github.com/zsviczian)
	-  Every single drawing or diagram is created using this Obsidian plugin.
3. [Quartz](https://quartz.jzhao.xyz/) by [Jacky Zhao](https://github.com/jackyzha0)
	- this is the static-site generator that helped me build the website and not worry about dealing with frontend code.